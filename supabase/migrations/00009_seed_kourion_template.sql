...
- **Rival City-Kingdoms**: Kition and Salamis aggression
  ...
  -- ============================================================================
  -- PROCESS STAGES (from KING_Process.csv - 12 phases, 120 minutes total)
  -- ============================================================================
  jsonb_build_array(
    jsonb_build_object('sequence', 1, 'name', 'Clan Councils 1', 'description', 'Each clan meets privately to choose strategy and nominate candidates', 'default_duration_minutes', 10),
    jsonb_build_object('sequence', 2, 'name', 'Free Consultations 1', 'description', 'All clans may discuss negotiate form alliances', 'default_duration_minutes', 15),
    jsonb_build_object('sequence', 3, 'name', 'Clans nominate candidates (decision)', 'description', 'Each clan announces their nominated candidate', 'default_duration_minutes', 5),
    jsonb_build_object('sequence', 4, 'name', 'Candidate Speeches 1', 'description', 'Each candidate has 2 minutes to present their program', 'default_duration_minutes', 15),
    jsonb_build_object('sequence', 5, 'name', 'Vote 1', 'description', 'Open voting - need 10+ votes to win. If no winner two candidates with highest votes proceed', 'default_duration_minutes', 10),
    jsonb_build_object('sequence', 6, 'name', 'Clan Councils 2', 'description', 'Each clan meets privately to discuss strategy for second round', 'default_duration_minutes', 10),
    jsonb_build_object('sequence', 7, 'name', 'Free Consultations 2', 'description', 'Final discussions between clans', 'default_duration_minutes', 15),
    jsonb_build_object('sequence', 8, 'name', 'Candidate Speeches 2', 'description', 'Top 2 candidates get 3 minutes each for final speeches', 'default_duration_minutes', 10),
    jsonb_build_object('sequence', 9, 'name', 'Vote 2', 'description', 'Open voting between final 2 candidates', 'default_duration_minutes', 5),
    jsonb_build_object('sequence', 10, 'name', 'King''s Decisions and Final Speech (if elected)', 'description', 'New King announces key decisions and policies', 'default_duration_minutes', 10),
    jsonb_build_object('sequence', 11, 'name', 'Clan Councils 3', 'description', 'Each clan meets to discuss their response to King''s decisions', 'default_duration_minutes', 8),
    jsonb_build_object('sequence', 12, 'name', 'Clan''s Final Decisions', 'description', 'Final comments and clan reactions to King''s announcements', 'default_duration_minutes', 7)
  ),
